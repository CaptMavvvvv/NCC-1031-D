...
def find_alternative_paths(start, destination, fastest_path):
    all_paths = list(nx.all_simple_paths(G, source=start, target=destination))
    alternative_paths = [path for path in all_paths if path != fastest_path]
    alternative_paths.sort(key=lambda path: sum(G[path[i]][path[i+1]]["car"] for i in range(len(path) - 1)))
    return alternative_paths[:3]

def generate_map(paths):
    start_coords = locations[paths[0][0]][0:2]
    ...
    for i, path in enumerate(paths):
        ...
        folium.PolyLine(route_coords, color=colors[i % len(colors)], weight=5).add_to(route_map)
        for mall in path:
...
def find_shortest_path():
    ...
    fastest_path, fastest_distance = find_fastest_route(start, destination, vehicle, day_type)

    if not fastest_path:
        messagebox.showerror("Error", "No available routes.")
        return

    alternative_paths = find_alternative_paths(start, destination, fastest_path)

    result = f"Fastest Route: {start} ➝ {' ➝ '.join(fastest_path)} (Distance: {fastest_distance:.1f} km)\n\n"
    paths_with_distances = [(fastest_path, fastest_distance)]

    if alternative_paths:
        result += "Alternative Routes:\n"
        for path in alternative_paths:
            distance = sum(G[path[i]][path[i+1]]["car"] for i in range(len(path) - 1))
            result += f" ➝ {' ➝ '.join(path)} (Distance: {distance:.1f} km)\n"
            paths_with_distances.append((path, distance))

    result_text.set(result)
    draw_graph(highlight_paths=[p[0] for p in paths_with_distances])
    if messagebox.askyesno("Generate Map", "Do you want to generate a map for these routes?"):
        generate_map([p[0] for p in paths_with_distances])

def add_node():
    mall_name = simpledialog.askstring("Add Mall", "Enter Mall Name:")
    if mall_name:
        lat = simpledialog.askfloat("Add Mall", "Enter Latitude:")
        lon = simpledialog.askfloat("Add Mall", "Enter Longitude:")
        province = simpledialog.askstring("Add Mall", "Enter Province:")
        if lat is not None and lon is not None and province:
            locations[mall_name] = (lat, lon, province)
            G.add_node(mall_name, pos=(lon, lat), province=province)
            start_menu["values"] = list(locations.keys())
            destination_menu["values"] = list(locations.keys())
            draw_graph()
            messagebox.showinfo("Mall Added", f"{mall_name} has been added.")
...
add_node_button = tk.Button(input_frame, text="+ Add Mall", font=("Arial", 12), command=add_node)
find_route_button.grid(row=4, column=0, columnspan=2, pady=10)

add_node_button = tk.Button(input_frame, text="+ Add Mall", font=("Arial", 12), command=add_node)
add_node_button.grid(row=5, column=0, columnspan=2, pady=10)
...
def draw_graph(highlight_path=None, highlight_paths=None):
    ax.clear()
    ...
    if highlight_path:
        ...
        nx.draw_networkx_edges(G, pos, ax=ax, edgelist=path_edges, edge_color="blue", width=3)
    if highlight_paths:
        colors = ["blue", "red", "green", "purple", "orange", "brown"]
        for i, path in enumerate(highlight_paths):
            path_edges = list(zip(path, path[1:]))
            nx.draw_networkx_edges(G, pos, ax=ax, edgelist=path_edges, edge_color=colors[i % len(colors)], width=3)

    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Bangkok', markerfacecolor='orange', markersize=10),
        Line2D([0], [0], marker='o', color='w', label='Nonthaburi', markerfacecolor='lightblue', markersize=10),
        Line2D([0], [0], marker='o', color='w', label='Samut Prakan', markerfacecolor='green', markersize=10),
        Line2D([0], [0], marker='o', color='w', label='Samut Sakhon', markerfacecolor='red', markersize=10),
    ]
